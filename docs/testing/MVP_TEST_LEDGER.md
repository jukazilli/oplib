...
- [ ] `Preview/WAF`: com autenticação Vercel renovada na equipe correta, registrar `firewall overview`, regras expandidas e `firewall diff`; confirmar a regra histórica de observação de login, ausência de draft esquecido e nenhuma regra de bloqueio publicada sem revisão humana.
