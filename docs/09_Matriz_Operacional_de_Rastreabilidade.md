...
| PUB-004 | D05 §§15–16; D07 §§11,18                   | publicar e atualizar atomicamente                           | `src/app/admin/publicacoes/actions.ts`, `src/modules/publishing/draft-repository.ts`, composer e invalidação de paths | prova transacional e navegação autenticada pendentes                                                    | `TEST-PUB-004-*` | `EVID-PUB-004-01`                  | `in_progress` |
...
| QUAL-002 | D03 §9; D06 §17; D07 §§5,18,20        | validar desempenho e degradação segura    | testes de falha, Lighthouse e análise de bundle          | —                                                                                                                                                                                                                     | `TEST-QUAL-002-*` | `EVID-QUAL-002-01`                 | `in_progress`   |
| SEC-001  | D02 §11; D06 §§14–15,18.4; D07 §24    | executar regressão de segurança           | testes unitários de segurança/rotas, CI, Vercel Firewall | corpus local cobre proveniência, autorização negativa, payload, upload, conteúdo não confiável, SQL parametrizado, rate limit, segredos e dependências sem alerta conhecido; execução remota, Preview e WAF pendentes | `TEST-SEC-001-*`  | `docs/evidence/EVID-SEC-001-01.md` | `in_progress`   |
