...
| PUB-004 | D05 §§15–16; D07 §§11,18                   | publicar e atualizar atomicamente                           | `src/app/admin/publicacoes/actions.ts`, `src/modules/publishing/draft-repository.ts`, composer e invalidação de paths | falso conflito temporal corrigido nos rascunhos existentes; publicação autenticada, concorrência e rollback pendentes | `TEST-PUB-004-*` | `docs/evidence/EVID-PUB-004-01.md` | `in_progress` |
| PUB-005 | D05 §17; D07 §§11,18                       | retirar e republicar, invalidando cache                     | `src/modules/publishing/draft-repository.ts`, `src/app/admin/publicacoes/actions.ts`, lista editorial                 | transições transacionais, auditoria, cache e ações por estado implementadas; Preview pendente           | `TEST-PUB-005-*` | `docs/evidence/EVID-PUB-005-01.md` | `in_progress` |
| PUB-006 | D02 §§8.1,9; D04 §14; D07 §9.1             | definir destaque editorial                                  | `src/modules/publishing/`, home cache                                                                                 | comando auditado, UI e seleção pública determinística implementados; Preview pendente                   | `TEST-PUB-006-*` | `docs/evidence/EVID-PUB-006-01.md` | `in_progress` |
...
| ID      | Origem                                  | Requisito                                      | Destino previsto                                                    | Implementação atual                                                                                        | Testes previstos | Evidência esperada                 | Status        |
| ------- | --------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ---------------- | ---------------------------------- | ------------- |
| WEB-001 | D04 §13; D05 §3; PUX 001–005            | criar shell público responsivo                 | `src/app/(public)/layout.tsx`, `src/components/editorial/`          | shell, skip link, localização e navegação responsiva implementados; Preview pendente                       | `TEST-WEB-001-*` | `docs/evidence/EVID-WEB-001-01.md` | `in_progress` |
| WEB-002 | D02 §8.1; D04 §14; D05 §4               | criar página inicial editorial                 | `src/app/(public)/page.tsx`, `src/modules/discovery/`               | hero, busca, destaques, recentes, áreas públicas e falhas isoladas implementados; Preview pendente         | `TEST-WEB-002-*` | `docs/evidence/EVID-WEB-002-01.md` | `in_progress` |
| WEB-003 | D02 §§8.1–8.2; D05 §5; D07 §13          | implementar acervo, busca, filtros e paginação | `src/app/(public)/publicacoes/`, `src/modules/discovery/`           | busca/filtros PostgreSQL, URL, Feed/Grade, paginação e estados implementados; Preview pendente             | `TEST-WEB-003-*` | `docs/evidence/EVID-WEB-003-01.md` | `in_progress` |
| WEB-004 | D02 §8.3; D04 §16; D05 §6               | criar página de leitura da publicação          | `src/app/(public)/publicacoes/[slug]/`, consulta pública e Markdown | consulta por status, leitura, indisponibilidade genérica e conteúdo seguro implementados; Preview pendente | `TEST-WEB-004-*` | `docs/evidence/EVID-WEB-004-01.md` | `in_progress` |
| WEB-005 | D02 §12; D05 §7                         | compartilhar por recurso nativo ou copiar link | `src/components/editorial/share-action.tsx`                         | Web Share API, fallback clipboard, feedback e URL canônica implementados                                   | `TEST-WEB-005-*` | `docs/evidence/EVID-WEB-005-01.md` | `in_progress` |
...
| SEO-001 | D02 §12; TL §17                         | gerar metadata, canonical e prévia social      | Metadata API nas rotas públicas                                     | política por ambiente, metadata global/dinâmica, canonical, capa e fallback social implementados           | `TEST-SEO-001-*` | `docs/evidence/EVID-SEO-001-01.md` | `in_progress` |
| SEO-002 | D02 §12; D07 §17; TL §17                | gerar sitemap, robots e dados estruturados     | `src/app/sitemap.ts`, `robots.ts`, JSON-LD                          | sitemap público, robots por ambiente, invalidação e Article JSON-LD implementados                          | `TEST-SEO-002-*` | `docs/evidence/EVID-SEO-002-01.md` | `in_progress` |

## 10. Interações e moderação

| ID       | Origem                            | Requisito                                 | Destino previsto                                            | Implementação atual                                                                | Testes previstos  | Evidência esperada                  | Status              |
| -------- | --------------------------------- | ----------------------------------------- | ----------------------------------------------------------- | ---------------------------------------------------------------------------------- | ----------------- | ----------------------------------- | ------------------- |
| LIKE-001 | D02 §8.5; D05 §8; D07 §14; TL §12 | curtir uma vez por navegador sem desfazer | `src/modules/interactions/likes/`, Route Handler, migration | cookie opaco, HMAC, inserção idempotente, contador autoritativo e UI implementados | `TEST-LIKE-001-*` | `docs/evidence/EVID-LIKE-001-01.md` | `in_progress`       |
| COM-001  | D02 §8.6; D05 §9; D07 §15; TL §12 | publicar e listar comentário imediato     | `src/modules/interactions/comments/`, Route Handler, UI     | —                                                                                  | `TEST-COM-001-*`  | `EVID-COM-001-01`                   | `implemented-local` |
| MOD-001  | D02 §8.7; D04 §23; D05 §21        | consultar comentários na administração    | `src/app/admin/comentarios/`                                | —                                                                                  | `TEST-MOD-001-*`  | `EVID-MOD-001-01`                   | `implemented-local` |
| MOD-002  | D02 §8.7; D05 §21; D07 §§11,15    | ocultar e restaurar comentário            | `src/modules/interactions/moderation/`                      | —                                                                                  | `TEST-MOD-002-*`  | `EVID-MOD-002-01`                   | `implemented-local` |
...
| ID       | Origem                                | Requisito                                 | Destino previsto                                         | Implementação atual                                                                                                                                                                                                   | Testes previstos  | Evidência esperada                 | Status          |
| -------- | ------------------------------------- | ----------------------------------------- | -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ---------------------------------- | --------------- |
| UX-001   | PUX 008–011,016; D05 §§22–25          | completar estados, feedback e recuperação | componentes/fluxos afetados + E2E                        | —                                                                                                                                                                                                                     | `TEST-UX-001-*`   | `EVID-UX-001-01`                   | `in_progress`   |
| QUAL-001 | PUX 012–013; D04 §§30–31; D05 §24     | validar acessibilidade e responsividade   | `tests/e2e/foundation.spec.ts`, checklist manual         | rotas públicas sem conteúdo representativo cobertas em 320/768/1440 px, axe A/AA, skip link e movimento reduzido; jornadas completas e validação humana pendentes                                                     | `TEST-QUAL-001-*` | `EVID-QUAL-001-01`                 | `in_progress`   |
| QUAL-002 | D03 §9; D06 §17; D07 §§5,18,20        | validar desempenho e degradação segura    | testes de falha, Lighthouse e análise de bundle          | degradação de interações, redução do runtime público, streaming e baseline 96/95/94 com CLS 0 comprovados; conteúdo, mídia e consulta representativos pendentes                                                        | `TEST-QUAL-002-*` | `docs/evidence/EVID-QUAL-002-01.md` | `in_progress` |
| SEC-001  | D02 §11; D06 §§14–15,18.4; D07 §24    | executar regressão de segurança           | testes unitários de segurança/rotas, CI, Vercel Firewall | corpus local e negativo no Preview aprovados, dependências sem alerta conhecido e auditoria WAF saneada preparada; configuração ativa, draft e rate limits distribuídos pendentes                                | `TEST-SEC-001-*`  | `docs/evidence/EVID-SEC-001-01.md` | `in_progress` |
...
| GATE-EDITORIAL    | AUTH-001/002, ADM-001, UX-002, TAX-001, PUB-001–005/007, MED-001, AUD-001, WEB-001/004  | `docs/evidence/GATE-EDITORIAL.md`    | `planned` |
