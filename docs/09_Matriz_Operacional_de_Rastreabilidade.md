...
| `in-progress` | 1 | FND-002 em execução |
...
| FND-002 | TL §§4–6,20; INF §24 | materializar Next.js, Node, TypeScript e pnpm | `package.json`, lockfile, `src/app/`, configs raiz | `package.json`, `.nvmrc`, `.node-version`, `tsconfig.json`, `next.config.ts`, `src/app/` | `TEST-FND-002-*` | `EVID-FND-002-01` | `in-progress` |
...
| FND-007 | D02 §13; TL §§5–6; INF §§7,10 | vincular Vercel em `gru1` | Vercel, `vercel.json`, `docs/runbooks/vercel.md` | — | `TEST-FND-007-*` | `EVID-FND-007-01` | `planned` |
| FND-008 | D07 §16; TL §8; INF §13 | configurar Clerk, allowlist e MFA | Clerk + `src/modules/identity/` | — | `TEST-FND-008-*` | `EVID-FND-008-01` | `planned` |
| FND-009 | D02 §8.4; D07 §19; INF §14 | configurar Blob público para capas | Vercel Blob + `src/modules/media/` | — | `TEST-FND-009-*` | `EVID-FND-009-01` | `planned` |
| FND-010 | D06 §20; TL §20; INF §16 | configurar CI de pull request | `.github/workflows/ci.yml` | — | `TEST-FND-010-*` | `EVID-FND-010-01` | `planned` |
| FND-011 | D06 §18.3; TL §19; INF §16 | configurar E2E, axe e smoke | `playwright.config.ts`, `tests/e2e/foundation.spec.ts` | — | `TEST-FND-011-*` | `EVID-FND-011-01` | `planned` |
| FND-012 | D06 §§20–21; INF §§16–17 | configurar release e rollback | `.github/workflows/release.yml`, runbook | — | `TEST-FND-012-*` | `EVID-FND-012-01` | `planned` |
| FND-013 | D02 §11; D07 §24; INF §19 | aplicar baseline de segurança | `src/lib/security/`, headers, Vercel Firewall | — | `TEST-FND-013-*` | `EVID-FND-013-01` | `planned` |
| FND-014 | D06 §13; D07 §21; INF §20 | configurar logs, health e alertas | `src/lib/observability/`, `src/app/api/health/route.ts` | — | `TEST-FND-014-*` | `EVID-FND-014-01` | `planned` |
| FND-015 | D07 §23; INF §§14–15 | criar backup criptografado e provar restore | `.github/workflows/backup.yml`, `scripts/`, runbook | — | `TEST-FND-015-*` | `EVID-FND-015-01` | `planned` |
| FND-016 | TL §26; INF §§25–26 | executar aceite integrado da Fundação | `docs/evidence/FND-016.md` | — | `TEST-FND-016-01` | `EVID-FND-016-01` | `planned` |

## 7. Identidade e administração

| ID | Origem | Requisito | Destino previsto | Implementação atual | Testes previstos | Evidência esperada | Status |
|---|---|---|---|---|---|---|---|
| AUTH-001 | D02 §9; D05 §10; D07 §16 | entrar e sair da administração | `src/app/(admin)/admin/`, `src/modules/identity/` | — | `TEST-AUTH-001-*` | `EVID-AUTH-001-01` | `planned` |
...
O item `FND-002 — Materializar toolchain e aplicação-base` está em execução. O próximo item planejado é `FND-003 — Criar baseline de UI e qualidade`. A implementação funcional somente poderá começar após `GATE-FND` ou exceção formalmente aprovada.
