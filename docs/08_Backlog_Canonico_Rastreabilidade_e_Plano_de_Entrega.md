...
Estado atual: implementação, testes locais, smoke do Preview e aceite autenticado do proprietário aprovados em `EVID-AUTH-001-01`.
...
Estado atual: proteção em duas camadas, matriz negativa, smoke do Preview e aceite humano de sessão expirada aprovados em `EVID-AUTH-002-01`. A persistência do rascunho editorial será validada quando o editor existir em PUB-001/PUB-002.
...
Estado atual: schema relacional existente preservado; seed determinístico executado duas vezes no Preview (`3` alterações e depois `0`); três áreas canônicas, associação múltipla, rejeição de duplicidade e rollback dos dados sintéticos comprovados; checks finais da PR #20 aprovados.
...
Estado atual: metadados, áreas e referências foram integrados ao rascunho e à prévia por revelação progressiva; o slug é normalizado, único e serializado por bloqueio transacional. Preview autenticado aceito pelo proprietário em 21/09/2026.
...
Estado atual: proveniência das mutações públicas, payload de comentários, autorização administrativa negativa, exposição de segredos, rotas de capa, conteúdo não confiável, consultas parametrizadas, rate limit local de comentários com memória limitada e gate de vulnerabilidades altas/críticas cobertos localmente; alertas moderados, execução remota, Preview e WAF continuam pendentes.
